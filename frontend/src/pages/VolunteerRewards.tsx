...
export const VolunteerRewards: React.FC = () => {
  ...
  const loadRewardsData = async () => {
    try {
      ...
      const [statsRes, catalogRes, redemptionsRes, transactionsRes] = await Promise.all([
        axios.get('/api/v1/analytics/volunteer'),
        axios.get('/api/v1/rewards'),
        axios.get('/api/v1/rewards/redemptions'),
        axios.get('/api/v1/volunteers/wallet/transactions')
      ]);
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              {catalog.map((itm) => {
                ...
                return (
                  <div key={itm.id} className="bg-white border-2 border-dashed border-natural-border rounded-2xl p-5 shadow-xs text-left flex flex-col justify-between space-y-5 hover:border-brand-400 transition-colors relative">
                    {/* Visual coupon notch */}
                    <div className="absolute -left-3 top-1/2 -mt-2.5 w-5 h-5 bg-[#FAF9F5] border-r border-natural-border rounded-full pointer-events-none"></div>
                    <div className="absolute -right-3 top-1/2 -mt-2.5 w-5 h-5 bg-[#FAF9F5] border-l border-natural-border rounded-full pointer-events-none"></div>
          ...
          {/* Right panel: Ledger History of Wallet Transactions */}
          <div className="lg:col-span-5 bg-white border border-natural-border rounded-2xl p-5 shadow-xs text-left flex flex-col space-y-4">
            <div className="border-b border-natural-border pb-3 flex items-center gap-1.5">
              ...
              <div>
                <h3 className="font-display font-bold text-xs uppercase tracking-wider text-natural-text">Wallet Activity Ledger</h3>
                <p className="text-[10px] text-natural-muted mt-0.5 font-semibold">Real-time ledger audit entries of credits and redemptions</p>
              </div>
            </div>

            {transactions.length === 0 ? (
              <div className="p-8 text-center text-natural-muted text-xs font-semibold">
                No wallet transactions recorded yet. Complete verified deliveries to earn points!
              </div>
            ) : (
              <div className="divide-y divide-natural-border max-h-[380px] overflow-y-auto pr-1">
                {transactions.map((tx) => {
                  ...
                  return (
                    <div key={tx.id} className="py-3.5 flex justify-between items-start text-xs gap-3">
                      <div className="min-w-0">
                        <strong className="text-natural-text block font-bold leading-normal truncate">{tx.description}</strong>
                        <span className="text-[9px] text-natural-muted block mt-1 font-semibold flex items-center gap-1">
                          <Clock className="w-3 h-3 text-natural-muted" /> {new Date(tx.timestamp || tx.timestamp).toLocaleDateString()} {new Date(tx.timestamp || tx.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                      <div className="text-right shrink-0">
                        <span className={`text-xs font-mono font-black rounded px-2 py-0.5 block ${
                          isEarn ? 'text-brand-700 bg-brand-50 border border-brand-100' : 'text-red-700 bg-red-50 border border-red-100'
                        }`}>
                          {isEarn ? `+${tx.amount}` : tx.amount} coins
                        </span>
                        <span className="text-[8px] text-natural-muted font-bold block mt-1 uppercase font-mono">
                          {tx.transactionType}
                        </span>
                      </div>
                    </div>
