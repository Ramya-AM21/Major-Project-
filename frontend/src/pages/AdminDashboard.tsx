...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'SHELTERS'>('OVERVIEW');
      ...
      <div className="border-b border-gray-200">
        <nav className="flex space-x-8">
          <button
            ...
            className={`pb-4 px-1 border-b-2 font-medium text-xs uppercase tracking-wider font-mono transition-all duration-150 ${
              activeTab === 'OVERVIEW'
                ? 'border-brand-650 text-brand-650 font-black'
          ...
          >
            Shelter Verifications ({pendingShelters.length})
          </button>
        </nav>
      </div>
      ...
      {activeTab === 'OVERVIEW' ? (
        ...
        <div className="bg-white border border-natural-border rounded-2xl p-6 shadow-xs text-left space-y-6">
          ...
          {sheltersLoading ? (
            ...
            <div className="p-8 text-center text-natural-muted font-semibold text-xs bg-gray-50 rounded-xl">No pending shelter verifications. All caught up!</div>
          ) : (
